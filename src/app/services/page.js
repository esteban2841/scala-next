...
export default function ServicesPage() {
  ...
  const { heroImg, headline, description, groups } = services.servicesData[activeTab];
  ...
  const nextTab =
    currentIndex < services.tabs.length - 1 ? services.tabs[currentIndex + 1] : null;

  return (
    <div className="relative flex flex-col items-center justify-start min-h-screen px-4 sm:px-6 lg:px-10 py-5 bg-primary text-secondary">
      {/* Category Tabs */}
      ...
      <div className="w-full px-4 sm:px-6 lg:px-10">
        <img
          src={heroImg}
          alt={activeTab}
          className="w-full max-h-96 object-cover rounded-lg mb-6 mt-10"
        />
      </div>

      {/* Headline */}
      <div className="w-full px-4 sm:px-6 lg:px-10 bg-secondary py-3 mb-10">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-10 mb-20">
        <p className="max-w-4xl mx-auto text-lg leading-loose text-justify [text-align-last:center]">
          {description}
        </p>
      </div>
              ...
              <>
                ...
                <div className="w-full text-center space-y-6 md:justify-self-end">
                  {group.items.map((step) => (
                    <div key={step.number}>
                      <h3 className="flex text-3xl md:text-4xl font-bold justify-between">
                        <span className="text-secondary mr-2">
                          {step.number}
                        </span>
                        {step.title}
                      </h3>
                      <p className="mt-4 text-lg md:text-3x1 leading-loose text-justify">
                        {step.text}
                      </p>
                    </div>
                  ))}
